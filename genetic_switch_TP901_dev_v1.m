...
%tids_array=[1:1:Tid*60/delta_t];%/delta_t]; %lidt mærkeligt system, sorry...
% Omregning af "tid" til sekunder. Derefter divison med vores tidsskridt
% altså delta_t. Dette giver antal opdateringer indtil ønskede "tid"

%Estimeret antal tidstrin
tids_array=[1:1:Tid];% lidt mærkeligt system, sorry...
Tid_real=zeros(1,length(tids_array));
% Grænse værdi for CI_tot(1) og PL dominans. 19.36-19.37
...
for l=1:runs
    ...
    hold on
    %NEDENSTÅENDE KAN SLETTES. PRODUCERET d. 2. april - Pacal
...
hold off;
% Prøv med scaling constrained, og plot en ret linje
xlabel('CItotal (nM)');
ylabel('MORtotal (nM)');
